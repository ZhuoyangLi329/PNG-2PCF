...
并显式采用“1Gpc/3Gpc 不同的 PK 拟合 kmin”，同时输出“三子图”版本：
1) 1Gpc 单独对比
2) 3Gpc 单独对比
3) 1Gpc+3Gpc 全部叠加

输出:
- 图: task5_v12_fnl0_pk2xi_boxkmin_kmax20_jointpanel.png
- 文本: task5_v12_fnl0_pk2xi_boxkmin_kmax20_jointpanel_summary.txt
"""
...
OUT_PNG = os.path.join(OUT_DIR, "task5_v12_fnl0_pk2xi_boxkmin_kmax20_jointpanel.png")
OUT_TXT = os.path.join(OUT_DIR, "task5_v12_fnl0_pk2xi_boxkmin_kmax20_jointpanel_summary.txt")
...
# model.ipynb 同款 FFTLog 设定（本版: box-specific kmin + fixed kmax=20）
K_INT_MAX = 20.0
FFTLOG_N = 4096
FFTLOG_PADDING = 4.0
FFTLOG_MU = 0.5
...
def build_fit_mask(data: Dataset, fit_kmin: float) -> np.ndarray:
    """构建 PK 拟合掩膜：支持按盒子设置不同 kmin。"""
    mask = (data.kcen >= fit_kmin) & (data.kcen <= PK_FIT_KMAX)
    # 防止协方差奇异：剔除零方差或非有限 bin
    p0_std = data.p0_std
    mask &= np.isfinite(p0_std) & (p0_std > 0.0)
    return mask


def fit_best_pk(data: Dataset, fit_kmin: float) -> Dict[str, float]:
    mask = build_fit_mask(data, fit_kmin=fit_kmin)
    if mask.sum() < 5:
...
def model_xi_on_s(bestfit: Dict[str, float], s_data: np.ndarray, k_int_min: float) -> np.ndarray:
...
def run_one_box(cfg: BoxConfig) -> Dict[str, object]:
    ...
    fit_mask = build_fit_mask(data, fit_kmin=cfg.fit_kmin)
    bestfit = fit_best_pk(data, fit_kmin=cfg.fit_kmin)
    xi_model = model_xi_on_s(bestfit, data.s, k_int_min=cfg.fit_kmin)
    metrics = compute_metrics(data.s, data.xi_mean, data.xi_std, xi_model)
    fit_info = {
        "kmin_target": float(cfg.fit_kmin),
        "kmin_used": float(np.min(data.kcen[fit_mask])),
        "kmax_used": float(np.max(data.kcen[fit_mask])),
        "n_kfit": int(np.sum(fit_mask)),
        "kint_min": float(cfg.fit_kmin),
        "kint_max": float(K_INT_MAX),
    }
    return {
        "config": cfg,
        "data": data,
...
def plot_joint(result_1gpc: Dict[str, object], result_3gpc: Dict[str, object]) -> None:
    fig, axes = plt.subplots(1, 3, figsize=(18.0, 5.6), sharex=True, sharey=True)

    # 前两个子图：分别比较 1Gpc / 3Gpc 的测量与模型
    for col, res in enumerate([result_1gpc, result_3gpc]):
        ...
        text = (
            f"kfit=[{fit_info['kmin_used']:.4f},{fit_info['kmax_used']:.4f}] (target kmin={fit_info['kmin_target']:.4f})\n"
            f"kint=[{fit_info['kint_min']:.4f},{fit_info['kint_max']:.1f}] + taper\n"
            f"PK_FIT_KMAX={PK_FIT_KMAX:.3f}, p={FIXED_P:.1f}, Nk={fit_info['n_kfit']}\n"
    ...
    ax3 = axes[2]
    styles = [
        (result_1gpc, "tab:blue", "1Gpc"),
        (result_3gpc, "tab:orange", "3Gpc"),
    ]
    for res, color, label in styles:
        data = res["data"]
        xi_model = res["xi_model"]
        s = data.s
        r2 = s**2
        ...
        ax3.plot(s, y_model, "-", lw=2.0, color=color, label=f"{label} model")

    ax3.set_title("1Gpc + 3Gpc overlay")
    ax3.set_xlabel(r"$r\ [h^{-1}{\rm Mpc}]$")
    ax3.grid(alpha=0.25)
    ax3.legend(loc="best", frameon=False, fontsize=8.5)

    fig.suptitle("fnl=0: best-fit PK -> 2PCF (separate panels + joint overlay)", y=0.995)
    fig.tight_layout()
...
def write_summary(result_1gpc: Dict[str, object], result_3gpc: Dict[str, object]) -> None:
    def line_for(res: Dict[str, object]) -> str:
        ...
        return (
            ...
            f"  kfit: target_kmin={fit_info['kmin_target']:.6f}, used=[{fit_info['kmin_used']:.6f}, {fit_info['kmax_used']:.6f}], Nk={fit_info['n_kfit']}\n"
            f"  kint: [{fit_info['kint_min']:.6f}, {fit_info['kint_max']:.6f}] + taper\n"
    ...
    with open(OUT_TXT, "w", encoding="utf-8") as f:
        f.write("任务5 v12: fnl0 下 1Gpc/3Gpc 的 best-fit PK -> 2PCF 一致性检查（box-kmin, kmax=20, taper保留）\n")
        f.write("========================================================\n")
        f.write(f"方法: 对齐 model.ipynb (PK best-fit + FFTLog, k_int按盒子设置, kmax={K_INT_MAX}, taper={EDGE_TAPER_FRAC})\n")
        f.write(f"拟合: PK_FIT_KMAX={PK_FIT_KMAX}, 固定 p={FIXED_P}, sn0={FIXED_SN0}, sigmas 自由\n")
        f.write("      且允许不同盒子使用不同 kmin（并自动剔除零方差 bin）\n\n")
