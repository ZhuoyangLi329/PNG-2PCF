...
脚本名称
--------
task5_v13_1gpc_joint_fnl0_fnl100_testb_scan.py

代码大纲（执行逻辑）
--------------------
第 0 部分：参数区
- 定义 1Gpc fnl0/fnl100 数据路径、拟合设置、积分设置、窗口扫描列表与输出路径。

第 1 部分：基础函数
- 提供“安全版 PK 拟合”（自动剔除零方差 k-bin，避免协方差奇异）。
- 提供 1Gpc 版 TestB 评估函数（窗口作用于总 P0，k_f 使用 1Gpc 盒长）。
...
# 1Gpc 数据（fnl100 / fnl0）
PK_1GPC_FNL100_GLOB = "/pscratch/sd/l/lzy/1gpc-UNIT-fnl100-fastpm/pk_masscut/pk_rsd_N*.dat"
PCF_1GPC_FNL100_GLOB = "/pscratch/sd/l/lzy/1gpc-UNIT-fnl100-fastpm/pcf_masscut/pcf_rsd_N*.dat"
PK_1GPC_FNL0_GLOB = "/pscratch/sd/l/lzy/1gpc-UNIT-fnl100-fastpm/pk_masscut_fnl0/pk_rsd_N*.dat"
...
# 输出
OUT_FIG1 = os.path.join(MISSION5_DIR, "task5_v13_1gpc_joint_window_tradeoff.png")
OUT_FIG2 = os.path.join(MISSION5_DIR, "task5_v13_1gpc_joint_window_best_curves.png")
OUT_TXT = os.path.join(MISSION5_DIR, "task5_v13_1gpc_joint_window_summary.txt")
...
def fit_best_pk_safe(data: m5.MockData) -> Dict[str, object]:
    ...
    目的
    ----
    1Gpc fnl0 的最小 k-bin 可能出现“样本标准差为 0”的情况，
...
def evaluate_testb_1gpc_single_window(
    s_data: np.ndarray,
    k_grid: np.ndarray,
    p0_in: np.ndarray,
...
def main() -> None:
    ...
    try:
        ...
        xi100_baseline = evaluate_unwindowed_1gpc(s100, k_grid, p0_100, kmin=K_FUND_1GPC)
        xi100_testa = evaluate_unwindowed_1gpc(s100, k_grid, p0_100, kmin=KMIN_GLOBAL)
        ...
        xi0_baseline = evaluate_unwindowed_1gpc(s0, k_grid, p0_0, kmin=K_FUND_1GPC)
        xi0_testa = evaluate_unwindowed_1gpc(s0, k_grid, p0_0, kmin=KMIN_GLOBAL)
        ...
        for wtype, params in WINDOW_SCANS:
            for p in params:
                ...
                xi100 = evaluate_testb_1gpc_single_window(
                    s_data=s100,
                    k_grid=k_grid,
                    p0_in=p0_100,
                ...
                xi0 = evaluate_testb_1gpc_single_window(
                    s_data=s0,
                    k_grid=k_grid,
                    p0_in=p0_0,
        ...
        ax.set_title("Task5 v13: 1Gpc joint window scan (fnl0 + fnl100)")
        ...
        with open(OUT_TXT, "w", encoding="utf-8") as f:
            f.write("任务5 v13：1Gpc 联合窗口扫描（fnl0 + fnl100）\n")
            f.write("========================================\n\n")
            ...
            f.write("[设置]\n")
            f.write(f"- 数据: 1Gpc fnl100 N={data100.nmock}, rid={data100.realizations.min()}..{data100.realizations.max()}\n")
            f.write(f"- 数据: 1Gpc fnl0   N={data0.nmock}, rid={data0.realizations.min()}..{data0.realizations.max()}\n")
