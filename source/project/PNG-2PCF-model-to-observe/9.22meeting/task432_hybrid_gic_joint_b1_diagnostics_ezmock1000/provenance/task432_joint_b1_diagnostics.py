...
def initialize():
    """只读加载两套已冻结输入，检查数据和预测完全一致。"""
    global MODEL, DATA, COVS, FROZEN
    OUT.mkdir(parents=True,exist_ok=True)
    MODEL=configure('jaxpower')('joint','cubic')
    ...
    assert sha(OUTS['jaxpower']/'xi_emulator.npz')==sha(OUTS['ezmock1000']/'xi_emulator.npz')
...
def manifest():
    ...
    save(OUT/'run_manifest.json',{'host':socket.gethostname(),'cpu_affinity':sorted(os.sched_getaffinity(0)),'source':str(Path(__file__).resolve()),'source_sha256':sha(__file__),'inputs':{c:{str(p/name):sha(p/name) for name in ('frozen_inputs.npz','xi_emulator.npz','input_audit.json')} for c,p in OUTS.items()},'formal_contract':'9.18 P0 13 + P2 9 + xi0 26 + xi2 26; mandatory scalar GIC; C_single; diagnostics retain originals','outputs':[str(p) for p in OUT.glob('*.json')]})
