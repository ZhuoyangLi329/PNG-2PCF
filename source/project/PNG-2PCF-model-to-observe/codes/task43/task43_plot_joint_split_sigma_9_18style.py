...
MEETING_DIR = PROJECT_ROOT / "9.22meeting/task43_joint_b1_split_sigma"
VARIANTS = (
    ("p02", "p", r"$P_0+P_2$"),
    ("xi02", "xi", r"$\xi_0+\xi_2$"),
    ("joint", "joint", r"joint (shared $\sigma_s$)"),
    ("joint_split", "split", r"joint ($\sigma_{s,P}\neq\sigma_{s,\xi}$)"),
)
PLOT_COLORS = {**COLORS, "split": "#9467bd"}
...
def load_chain(root: Path, tag: str) -> dict[str, Any]:
    ...
    if summary.get("gates") and not all(summary["gates"].values()):
...
def triangle_page(
    ...
    *,
    ...
    for variant, color_key, variant_display in VARIANTS:
        ...
        handles.append(plt.Line2D([], [], color=PLOT_COLORS[color_key], lw=2.0))
...
def main() -> None:
    ...
    output = MEETING_DIR / f"task43_{ezlabel}_joint_shared_vs_split_sigma_9.18style.pdf"
    ...
    chains = {
        f"{cov}_{variant}": load_chain(
            split_root if variant == "joint_split" else mcmc_root,
            f"{cov}_joint_split_sigma" if variant == "joint_split" else f"{cov}_{variant}",
        )
        for cov, _ in pages
        for variant, _, _ in VARIANTS
    }
    diagnostic = mcmc_root / "diagnostics/joint_b1/task43_joint_b1_diagnostic.json"
    if diagnostic.is_file():
        payload = json.loads(diagnostic.read_text(encoding="utf-8"))
        for cov, _ in pages:
            theta = payload["covariances"][cov]["baseline"]["joint"]["theta"]
            chains[f"{cov}_joint"]["summary"]["map_theta"] = [
                float(theta["fNL"]), float(theta["b1"]), float(theta["sigma_s"]), float(theta["sn0"])
            ]
    ...
    for tag, entry in chains.items():
        ...
        constraints[tag] = {
            ...
            "gates": entry["summary"].get("gates", {"formal_split_chain": True}),
        }
    atomic_write_json(
        output_json,
        {
            "task": "task43_plot_joint_split_sigma_9_18style",
            "status": "pass",
            "mcmc_root": str(mcmc_root),
            "ezmock_realizations": int(arguments.nmock),
            "style_reference": "9.18meeting/task43_ezmock_covariance_mcmc_vs_jaxpower",
            ...
            "split_mcmc_root": str(split_root),
