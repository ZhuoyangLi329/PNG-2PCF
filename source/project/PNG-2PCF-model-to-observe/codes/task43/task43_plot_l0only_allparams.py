...
ROOT=PROJECT_ROOT; R=ROOT/'outputs/task43_outputs/ezmock_rsd_lightcone_covariance_z0p4_0p8_x1000_common50_nzmatch_b0.30_z0.60/mcmc_preliminary_900_l0only_verified'; OUTDIR=ROOT/'9.22meeting/task43_l0only'
PARAMS=('fNL','b1','sigma_s','sn0'); LABEL={'fNL':r'$f_{\rm NL}$','b1':r'$b_1$','sigma_s':r'$\sigma_s\ [h^{-1}{\rm Mpc}]$','sn0':r'$s_{n0}$'}; VAR=(('p0_only','P0 only',COLORS['p']),('xi0_only',r'$\xi$0 only',COLORS['xi']),('joint_p0_xi0','P0+$\\xi$0 joint',COLORS['joint']))
def load(v):
    with np.load(R/f'chain_ezmock900_{v}.npz',allow_pickle=False) as d: c=np.asarray(d['chain'],dtype='f8').reshape(-1,d['chain'].shape[-1])
    s=json.loads((R/f'summary_ezmock900_{v}.json').read_text()); names=tuple(s['names']); return {'chain':c,'summary':s,'names':names,'path':R/f'chain_ezmock900_{v}.npz'}
def val(e,p):
...
def main():
    ...
    plt.rcParams.update({'font.family':'sans-serif','font.sans-serif':['Arial','DejaVu Sans','Liberation Sans'],'pdf.fonttype':42,'ps.fonttype':42,'font.size':9.2,'axes.linewidth':1.,'xtick.direction':'in','ytick.direction':'in','xtick.top':True,'ytick.right':True}); OUTDIR.mkdir(parents=True,exist_ok=True); out=OUTDIR/'task43_ezmock900_l0only_allparams_9.18style.pdf'; tmp=out.with_name('.'+out.stem+'.tmp.pdf')
    with PdfPages(tmp) as pdf:
        ...
        fig.legend(handles,labels,loc='upper center',bbox_to_anchor=(.54,.995),ncol=3,frameon=False,fontsize=10.5,handletextpad=.5,columnspacing=1.); fig.suptitle('Task43 RSD lightcone: EZmock-900; monopole only ($\\ell=0$); free $s_{n0}$',fontsize=12,y=1.02); fig.text(.985,.005,'P0+\\xi0 only; P0/\\xi0/joint ownership shown by missing panels',ha='right',fontsize=8.3,color='.35'); fig.subplots_adjust(left=.08,right=.985,bottom=.065,top=.90,wspace=.06,hspace=.06); pdf.savefig(fig,bbox_inches='tight',pad_inches=.08); plt.close(fig)
    tmp.replace(out); audit={'task':'task43_ezmock900_l0only_allparams','status':'pass','multipoles':['ell0'],'excluded_multipoles':['ell2'],'free_parameters':PARAMS,'ownership':{'p0_only':['fNL','b1','sigma_s','sn0'],'xi0_only':['fNL','b1','sigma_s'],'joint_p0_xi0':['fNL','b1','sigma_s','sn0']},'chains':{v:str(e[v]['path']) for v,_,_ in VAR},'output_pdf':str(out),'output_pdf_sha256':sha256_file(out)}; atomic_write_json(out.with_suffix('.json'),audit); print(json.dumps(audit,sort_keys=True))
